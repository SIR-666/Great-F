...
export default function DisableFieldEditable({ evt, token }) {
  ...
  const optionsTipeMaterial = [
    ...
    { label: "Packaging Material", value: "PA" },
    ...
    try {
      await fetch("http://10.24.7.70:8080/siQmaMaterial", {
        ...
        .then((json2) => {
          ...
          console.log("code", code); //RM16
          console.log("values.nama_material ", values.tipe_material); //RM
          setKode(code);
          const dua = async () => {
            try {
              //http://10.24.7.70:8080/siQma/RM58
              await fetch(
                `http://10.24.7.70:8080/siQma2/${code}/${values.tipe_material}`,
                {
                  method: "GET",
                }
              )
