...
export default function DisableFieldEditable({ evt, token }) {
  ...
  const [columns, setColumns] = useState([
    ...
    { title: "Blending Formula", field: "Blending Formula", editable: "never" },
    ...
    {
      title: "Product Sample Used (lt)",
      field: "Product_Sample_Used_Lt",
      editable: "never",
    },
    {
      title: "Yield Category",
      field: "Yield_Category",
      editable: "never",
    },
    {
      title: "Blow Awal",
    ...
    {
      title: "Pengecekan",
      field: "PENGECEKAN_lt", //harusnya pengecekan2
      type: "numeric",
      // editable: "never",
    },
    {
      title: "Variable Loss",
      field: "Variable_loss", //harusnya VARIABLE_LOSS2
      type: "numeric",
      // editable: "never",
      render: (rowData) => {
        return rowData["Variable_loss"].toFixed(2);
      },
    },
    {
      title: "Fix Loss",
      ...
      render: (rowData) => {
        // console.log(rowData);
        return (
          parseFloat(rowData["Blow_awal"]) +
          parseFloat(rowData["Blow_akhir"]) +
          parseFloat(rowData["PENGECEKAN_lt"]) + //ubah jadi pengecekan2
          parseFloat(rowData["Product_Sample_Used_Lt"].toFixed(2))
        );
      },
    },
    {
    ...
    },
    {
      ...
      render: (rowData) => {
        // console.log(rowData);
        return parseInt(rowData["Loss_per_Flavour"]) -
          (parseFloat(rowData["Blow_awal"]) +
            parseFloat(rowData["Blow_akhir"]) +
            parseFloat(rowData["PENGECEKAN_lt"]) + //ubah jadi pengecekan2
            parseFloat(rowData["Product_Sample_Used_Lt"].toFixed(2))) -
          parseInt(rowData["Variable_loss"]) -
          parseInt(rowData["SIRKULASI"]) ==
          0 ? (
          parseInt(rowData["Loss_per_Flavour"]) -
            (parseFloat(rowData["Blow_awal"]) +
              parseFloat(rowData["Blow_akhir"]) +
              parseFloat(rowData["PENGECEKAN_lt"]) + //ubah jadi pengecekan2
              parseFloat(rowData["Product_Sample_Used_Lt"].toFixed(2))) -
  ...
  const handleSubmitModal = async (e) => {
    ...
    const arr = [];
    try {
      await fetch(`http://10.24.7.70:8080/GreatgetLossMilM/${date1}/${date2}`, {
        method: "GET",
      })
        .then((response) => {
          ...
          onRowUpdate: (newData, oldData) =>
            new Promise((resolve, reject) => {
              setTimeout(() => {
                ...
                console.log("newData  ", newArray); //ID database
                console.log("thn  ", newArray.AUFNR); //ID database
