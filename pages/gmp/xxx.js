...
export default function DisableFieldEditable({ evt, token }) {
  ...
  // let oidDateData = values.idDateData.substring(
  //   0,
  //   values.idDateData.indexOf("_")
  // );

  const [oidDateData, setoidDateData] = useState([]);
  ...
  const handleInputChange5 = (e) => {
    ...
    setValues({
      ...values,
      // idDateData: e.value,
      idDateData: e.value.substring(0, e.value.indexOf("_")) + "_" + jam,
    });
    setoidDateData(e.value);
  ...
  const handleSubmitModal = async (e) => {
    ...
    try {
      await fetch(`http://10.24.7.70:8080/siQma2/${arr}/${arr}`, {
        // await fetch(`http://10.24.7.70:8080/siQmaMilkEditD/${oidDateData}`, {
        method: "GET",
      })
        .then((response) => {
          return response.json(); // return response.json() first
        ...
        .then((json) => {
          ...
          const dua = async () => {
            try {
              ...
              await fetch(
                `http://10.24.7.70:8080/siQmaMilkEditH/${oidDateData}`,
                {
                  method: "GET",
                }
              )
                ...
                .then((json2) => {
                  ...
                  setValues({
                    ...values,
                    idDateData:
                      options[0].idDateData.substring(
                        0,
                        options[0].idDateData.indexOf("_")
                      ) +
                      "_" +
                      jam,
  ...
  return (
    <Layout title="siQma">
      ...
      <form onSubmit={handleSubmit} className={styles.form2}>
        ...
        <div className={styles.grid}>
          <div>
            <h3>#id: {values.idDateData} </h3>
            <h5>#Reference id: {oidDateData} </h5>
          </div>
          <div>
