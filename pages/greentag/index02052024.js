...
export default function AddEventPage({}) {
    ...
    if (values.ExpectedDate < values.IssuedDate) {
      ...
      try {
        ...
        if (recipients.length > 0) {
          // console.log("recipients 2 ", recipients);

          const emailData = {
            to: recipients,
            subject: `TRIAL - GreenTag - ${values.TagNo} - ${values.info1} - ${values.AbnormalityType} - ${values.MaintenanceType} - ${values.Line} - ${values.ExpectedDate}`,
            body: `
              Dear All,
  ...
  const determineRecipients = async (values) => {
    try {
      ...
      // console.log("values.ObservedArea ", values.ObservedArea);
