  ...
  useEffect(() => {
    async function fetchJumlah() {
      ...
      const response = await fetch(
        `http://10.24.7.70:8080/getgreenCountTAG/${year}`
      );
      const data = await response.json();
      var openData = data.find((item) => item.Remarks === "Open");
      var totalData = data.find((item) => item.Remarks === "Total");
      setdataOpen(openData.TAG);
      setdataAll(totalData.TAG);
      // console.log("data Open", openData.TAG);
      // console.log("data Total", totalData.TAG);
    }
    fetchJumlah();
  }, [dataOpen, dataAll]);
...
export async function getServerSideProps() {
  ...
  const d = new Date();
  let year = d.getFullYear();
  const res = await fetch(`http://10.24.7.70:8080/getgreenTAGallOpen/open`);
